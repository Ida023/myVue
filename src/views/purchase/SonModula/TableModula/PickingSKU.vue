      ...
      <el-table :data="TableData" border tooltip-effect="dark" style="width: 100%" :stripe="false"  @selection-change="selsChange">
        ...
        <el-table-column sortable label="体积（cm³）" align="center">
          <template scope="scope">
            <div class="simulation" v-for="(item,index) in scope.row.stockItems" :id="'simulation' +'-'  + scope.$index +'/' + index">
              <div class="childDiv">
                <span v-text="(item.productbulk * item.requestQty).toFixed(2)"></span>
              </div>
            </div>
          </template>
        </el-table-column>
...
//            alert('待供应链处理')
            ...
            this.HandleTasksSon.state=false;
            _this.doItem('您确定批量回退备货计划？（备货计划将重新提交第三方审核）','stockProductPlan/pacthStockItemState',_this.HandleTasksSon,_this.SonDifferentiatedRequest);
          }else if(val==='待提交采购'){
            this.HandleTasksSon.state=true;
...
//          alert('草稿保存')
          ...
          this.HandleTasksSon.state=false;
          _this.doItem('您确定回退此备货计划？（备货计划将重新提交待供应链处理）','stockProductPlan/pacthStockItemState',_this.HandleTasksSon, _this.SonDifferentiatedRequest);
        }else if(state==='库存'){
          this.$message('待开发')
        ...
        if(this.RemarksState==='同意'){
          this.httpPost('stockProductPlan/pacthStockItemState',_this.HandleTasksSon).then(res=>{
            if(res.data.result===true){
              ...
              _this.$emit('DifferentiatedRequest')
      ...
      CalculationHeight:function () {//合并表格 小行统一高度。
        ...
        for(let i=0;i<objList.length;i++) {
          ...
          let group = subName[0];//row 的index
          let groupRow = subName[1];//合并行的 index
//          console.log('heights length:', heights.length);
          if (heights.length < group + 1) {//heights 为空,添加一个空对象；
            this.$set(heights, group, []);
          }

          setTimeout(() => {
            if (heights[group].length < groupRow + 1) {//heights[]
