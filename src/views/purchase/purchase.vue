<template>
  <div class="main2">
    <div  class="subnav" :class="collapsed?'menu-collapsed':'menu-expanded'">
      <el-menu @select="selectFn" v-show="!collapsed" v-if="!collapsed"class="el-menu-vertical-demo el-menu-show " theme="dark" router :default-active="subNavv" :unique-opened="true">
        <el-submenu v-for="(item,index) in msg " :index="item.grpName" v-if="item.parentId==='02'" :key="index">
      ...
      <ul class="el-menu el-menu-hide el-menu-vertical-demo collapsed" ref="menuCollapsed" v-show="collapsed">
        <li v-for="(item,index) in msg " :index="item.grpName" v-if="item.parentId==='02'" class="subnavitem" :key="index">
          <template>
            ...
            <ul class="el-menu submenu sidebar-n" :class="'submenu-hook-'+index" @mouseover="showMenu(index,true)"
                @mouseout="showMenu(index,false)">
              <li v-for="(listurl,index) in item.groupMenuList" :index="listurl.menuUrl" class="el-menu-item" :key="index" style="padding-left: 40px;">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      selectFn:function (index, indexPath) {
