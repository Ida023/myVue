  ...
  <div id="outermost_wrap" class="user_manager_wrap">
    <div v-for="(city,index) in cityOptions" :key="index" style="margin-top:40px;" :class="'class-'+index">
      ...
      <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange(index)" :id="index">
        <el-checkbox v-for="(item,iIndex) in city.cityList" :label="item.id" :key="item.id" :value="item.id"></el-checkbox>
      </el-checkbox-group>
    </div>
  </div>
</template>
<script>
  export default{
    ...
    mounted () {
      var childrenList = document.getElementsByClassName('el-tree-node__children');
      setTimeout(function () {
        if (childrenList) {
          for (var i = 0; i < childrenList.length; i++) {
            if (!childrenList[i].children.length) {
              var parentNode = childrenList[i].parentNode.getElementsByClassName('btnGroup')[0];
              var childButton = parentNode.getElementsByClassName('addItem')[0];
              parentNode.removeChild(childButton);
            }
          }
        }
