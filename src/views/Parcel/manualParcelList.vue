    ...
    <div id="outermost_wrap">
        ...
        <div id="content_wrap">
              ...
              <el-collapse v-model="activeNames">
                <el-collapse-item name="1">
                  <el-row class="marginBottom">
                    ...
                    <el-radio-group v-model="pagefilters.whId" size="small"  @change="pagefilters.parcelNo='';getlogisticsName();">
                  ...
                  <el-row  class="marginBottom">
                    ...
                    <el-radio-group v-model="pagefilters.refId" size="small"  @change="pagefilters.parcelNo='';getParcelList();"  >
                      <el-radio-button v-for="item in logisticsName" :key="item.refId" :label="item.refId" :value="item.refId" >{{item.refValue}}</el-radio-button>
                    </el-radio-group>
                  </el-row>
                  <el-row  class="marginBottom">
                  <span class="span_label">状&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 态</span>
                  <el-radio-group v-model="pagefilters.status" size="small"  @change="getParcelList();">
                    <el-radio-button label="01">待生成跟踪单号</el-radio-button>
                    <el-radio-button label="02">已生成跟踪单号</el-radio-button>
                    <el-radio-button label="03">已打印邮票</el-radio-button>
          ...
          <el-row class="supplier_btn">
            <el-button type="primary" size="small" v-if= "pagefilters.status == '01'"  @click="ChangeList('','delete');">批量删除</el-button>
            <el-button type="primary" size="small"  v-if="pagefilters.status == '01'"  @click="ChangeList('','submit');">批量提交获取跟踪单号</el-button>
            <el-button type="primary" size="small"  v-if="pagefilters.status == '02'"  @click="ChangeList('','again');">重新获取</el-button>
            <!--<el-button type="primary" size="small"  v-if="pagefilters.whId != '01'">批量打印DHL邮票</el-button>-->
            <el-button type="primary" size="small"  v-if="pagefilters.status == '03'"  @click="ChangeList('','sign');">批量标记发货</el-button>
            <!--<el-button type="primary" size="small"  v-if="pagefilters.whId == '04'">批量打印回邮邮票</el-button>-->
            <!--<el-button type="primary" size="small"  v-if="pagefilters.whId == '04'" >批量打印报关单</el-button>-->
          </el-row>
          <div id="item_list">
            <el-table  :data="parcelList"  ref="parcelList"  border tooltip-effect="dark"   @selection-change="batchChange"  style="width:100%;">
              ...
              <el-table-column  id="space" label="操作"  align="center" style="word-wrap:break-word;word-break:break-all;">
                <template scope="scope">
                  ...
                  <el-tooltip content="编辑"   effect="dark" placement="bottom"  v-if="scope.row.status == '待生成跟踪单号'" >
                  ...
                  <el-tooltip content="提交获取跟踪单号"   effect="dark" placement="bottom"  v-if="scope.row.status == '待生成跟踪单号'" >
                    <i  class="iconfontyyy"  v-if="scope.row.status" content="Bottom center" @click="ChangeList(scope.row,'submit');">&#xe63e;</i>
                  </el-tooltip>
                  <el-tooltip content="删除"   effect="dark" placement="bottom"  v-if="scope.row.status == '已生成跟踪单号' || scope.row.status == '待生成跟踪单号' " >
                    <i  class="iconfontyyy"  content="Bottom center" @click="ChangeList(scope.row,'delete');">&#xe6f7;</i>
                  </el-tooltip>
                  <el-tooltip content="重新发起"   effect="dark" placement="bottom" v-if="scope.row.status == '已生成跟踪单号' || scope.row.status == '已打印邮票' ">
                    <i  class="iconfontyyy"  content="Bottom center" @click="ChangeList(scope.row,'again');">&#xe61c;</i>
                  </el-tooltip>
                  <el-tooltip content="标记发货"   effect="dark" placement="bottom" v-if="scope.row.status == '已打印邮票'">
      ...
      <el-dialog :title="dialogFlag.dialogTitle" :visible.sync="dialogFlag.isOpen" size="small"
        ...
        <el-tabs v-model="dialogFlag.activeTab" @tab-click="tabs">
          <el-tab-pane label="基础信息"  name="1" :disabled="dialogFlag.dialogDisabled" >
            <el-form labelWidth="120px" :model="msgData" ref="msgData" :rules="msgrules" >
              <el-col :span="12">
                ...
                <el-form-item label="重量（kg）" prop="parcelWeight" required  >
                  <el-input type="number" :disabled="dialogFlag.inputDisabled" v-model="msgData.parcelWeight">
                    <template slot="append">CM</template>
                  </el-input>
                </el-form-item>
                <el-form-item label="" prop="parcelLength">
                  <el-input type="number" :disabled="dialogFlag.inputDisabled" v-model="msgData.parcelLength">
                    <template slot="prepend">长</template>
                    <template slot="append">CM</template>
                  </el-input>
                </el-form-item>
                <el-form-item label="" prop="parcelWidth">
                  <el-input type="number" :disabled="dialogFlag.inputDisabled" v-model="msgData.parcelWidth">
                    <template slot="prepend">宽</template>
                    <template slot="append">CM</template>
                  </el-input>
                </el-form-item>
                <el-form-item label="" prop="parcelHeight">
                  <el-input type="number" :disabled="dialogFlag.inputDisabled" v-model="msgData.parcelHeight">
                    <template slot="prepend">高</template>
                    <template slot="append">CM</template>
                  </el-input>
                </el-form-item>
                <el-form-item label="周长" prop="">
                  {{msgData.parcelLength * 2 + msgData.parcelWidth * 2}}
                ...
                <el-form-item label="包裹个数" prop="parcelQty">
                  <el-input type="number" :disabled="pagefilters.refId === '02' ? false :true" v-model="msgData.parcelQty"></el-input>
                </el-form-item>
                <el-form-item label="包裹价值" prop="parcelValue" required >
                  <el-col :span="11" style="margin-right:5px;">
                    <el-input type="number" :disabled="dialogFlag.inputDisabled" v-model="msgData.parcelValue"></el-input>
                    <span ></span>
                  </el-col>
          ...
          <el-tab-pane label="发件人信息"  :disabled="dialogFlag.dialogDisabled" name="2">
            <msgform ref="addremsgform" :Disabled="dialogFlag.inputDisabled" :houseId="pagefilters.whId" :logisticsId="pagefilters.refId"
                     :getpoId="lineId" :typeName="type" v-on:child-change-Val="getChildvalue" :dialogTitle="dialogFlag.dialogTitle" ></msgform>
          </el-tab-pane>
          <el-tab-pane label="收件人信息" :disabled="dialogFlag.dialogDisabled"  name="3">
            <msgform ref="addremsgform1" @changecountry-value="changeValue" v-on:child-change-Val="getChildvalue"  :dialogTitle="dialogFlag.dialogTitle" :Disabled="dialogFlag.inputDisabled" :houseId="pagefilters.whId"
                     :logisticsId="pagefilters.refId" :getpoId="lineId" :typeName="type"></msgform>
          </el-tab-pane>
          <el-tab-pane label="包裹类型与服务" :disabled="dialogFlag.dialogDisabled"  name="4">
            <el-form :label-width="financeFormLabelWidth" :model="serviceData" ref="serviceData">
              <el-col>
                ...
                <el-form-item label="Sender‘s Instructions" prop="insuredAmount">
                  <el-col :span="15">
               ...
               <!-- <el-col style="margin-left: 50px;" v-if="pagefilters.refId =='01'">
                ...
                <el-col style="margin-left:40px;">
                  ...
                  <msgform ref="addremsgform2"   v-on:child-change-Val="getChildvalue"  :dialogTitle="dialogFlag.dialogTitle" v-if="isReturnAddress == true"   :Disabled="dialogFlag.inputDisabled" :typeName="issameAddress == true ? '发件人' : '联系人'" :logisticsId="pagefilters.refId" :getpoId="lineId" ></msgform>
                </el-col>
              </el-col>
            </el-form>
          </el-tab-pane>
        ...
        <div slot="footer" class="dialog-footer" v-if="dialogFlag.dialogDisabled== true">
          ...
          <el-button type="primary" @click="submitDialogForm();" v-if="dialogFlag.activeTab != '1'">保 存</el-button>
          <el-button type="primary" @click="submitDialogForm();" v-if="dialogFlag.activeTab != '4'">保 存&下一步</el-button>
        </div>
        <div slot="footer" class="dialog-footer" v-if="dialogFlag.dialogDisabled == false">
          <el-button @click="dialogFlag.isOpen = false;">取消</el-button>
          <el-button type="primary" @click="submitDialogForm();">保 存</el-button>
        </div>
      </el-dialog>
      <!--收件人国家非欧盟弹窗-->
      ...
      <el-dialog  title="常用海关编码" :visible.sync="dialogFlag.CustomsCodeisOpen" size="small" :close-on-click-modal="false" @close="dialogFlag.activeTab='1'" >
        ...
        <div slot="footer" class="dialog-footer" >
          ...
          <el-button type="primary" @click="submitDialogForm" :loading="$store.state.loading">确 定</el-button>
        </div>
      </el-dialog>
  ...
  export default{
    ...
    data(){
      ...
      getChildvalue:function(val){
        ...
        val.type=='发件人'?this.addSender = obj:val.type=='收件人'?this.addAddressee=obj:val.type == '联系人'?this.serviceData.addresser=obj:'';
        if(val.type=='发件人' && this.issameAddress == true ){this.serviceData.addresser=obj}
      ...
      tabs(val){
        this.dialogFlag.activeTab== '2' ? this.type='发件人' : this.dialogFlag.activeTab=='3' ? this.type='收件人':'';
      ...
      mubmitserviceData(){
        ...
        this.serviceData.refId=this.pagefilters.refId;
      ...
      goNext:function() {
        this.dialogFlag.activeTab = this.tabArr[this.tabArr.indexOf(this.dialogFlag.activeTab) + 1];
        this.type = this.dialogFlag.activeTab =='2' ? '发件人' :this.dialogFlag.activeTab =='3' ? '收件人':'';
      ...
      getlogisticsName(){
      	this.httpPost('manualPackage/type?whId='+this.pagefilters.whId).then(res=>{
            ...
            if(res.data.length > 1){
              this.pagefilters.refId=res.data[0].refId;
            }
            this. getParcelList();
