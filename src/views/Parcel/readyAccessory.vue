   ...
   <div id="outermost_wrap">
     ...
     <div id="content_wrap">
         ...
         <el-collapse v-model="activeNames">
           <el-collapse-item name="1">
             ...
             <el-radio-group v-model="pagefilters.whStatus" size="small"   @change="pagefilters.skuNo='';pagefilters.orderNo='';pagefilters.userId='';pagefilters.startDate ='';pagefilters.endDate ='';getAccessList();" >
               <el-radio-button label="01"    v-if="ButtonInfo['待发配件']!='0'">待发配件</el-radio-button>
               <el-radio-button label="04"    v-if="ButtonInfo['待从新品取配件发货(待审核)']!='0'">待从新品取配件发货(待审核)</el-radio-button>
               <el-radio-button label="02"   v-if="ButtonInfo['待从新品取配件发货(已审核)']!='0'" >待从新品取配件发货(已审核)</el-radio-button>
               <el-radio-button label="03"    v-if="ButtonInfo['已发配件']!='0'">已发配件</el-radio-button>
             </el-radio-group>
           </el-collapse-item>
         </el-collapse>
       </el-row>
       ...
       <el-row class="supplier_btn">
         <el-button type="primary" size="small" v-if="(pagefilters.whStatus == '01' || pagefilters.whStatus == '02') && ButtonInfo['批量设置物流方式']!='0'" @click="statusChange('','logist')" >批量设置物流方式</el-button>
         <el-button type="primary" size="small" v-if="(pagefilters.whStatus == '01' || pagefilters.whStatus == '02' )&& ButtonInfo['批量打印地址标签']!='0'" @click="">批量打印地址标签</el-button>
         <el-button type="primary" size="small" v-if="pagefilters.whStatus == '01' && ButtonInfo['批量从新品发货']!='0'" @click="statusChange('','status')" >批量从新品发货</el-button>
       </el-row>
       <div id="item_list">
         <el-table  :data="AccessoryData"  ref="AccessoryData"  border tooltip-effect="dark"  @selection-change="batchChange"   style="width:100%;" >
           ...
           <el-table-column  id="space" label="操作"  align="center" style="word-wrap:break-word;word-break:break-all;">
             <template scope="scope" >
               <el-tooltip content="设置状态"  v-if="scope.row.whStatus == '待发货'  && ButtonInfo['设置状态']!='0'" effect="dark" placement="bottom">
                 <i  class="iconfontyyy" @click="statusChange(scope.row,'status');" content="Bottom center">&#xe60d;</i>
               </el-tooltip>
               <el-tooltip content="设置物流方式"  effect="dark" v-if="(scope.row.whStatus == '待发货' || scope.row.whStatus == '待从新品发货') && ButtonInfo['设置物流方式']!='0'" placement="bottom">
                 <i  class="iconfontyyy" @click="statusChange(scope.row,'logist');" content="Bottom center">&#xe604;</i>
               </el-tooltip>
               <el-tooltip content="打印地址" v-if="scope.row.whStatus != '待审核' && ButtonInfo['打印地址']!='0'" effect="dark" placement="bottom" >
                 <i  class="iconfontyyy"      content="Bottom center">&#xe633;</i>
               </el-tooltip>
               <el-tooltip content="跟踪单号" v-if="(scope.row.whStatus == '待发货' || scope.row.whStatus == '待从新品发货') && ButtonInfo['跟踪单号']!='0'" effect="dark" placement="bottom" >
                 <i   class="iconfontyyy"    content="Bottom center" @click="TrackChange(scope.row)" >&#xe617;</i>
               </el-tooltip>
              <!--   <el-popover  v-if="scope.row.whStatus == '待发货' || scope.row.whStatus == '待从新品发货' "
                   placement="top"
                   width="150"
                   trigger="click"
                   popper-class="my-popover"
                  >
                   <el-button style="margin-bottom: 5px;" v-if="scope.row.isNeedTrackingNo == '0'" @click="TrackChange(scope.row,'1');">不需要跟踪单号</el-button>
                   <el-button style="margin-left:0;" v-if="scope.row.isNeedTrackingNo == '1'"  @click="TrackChange(scope.row,'2');">需要跟踪单号</el-button>
                   <i   class="iconfontyyy" slot="reference"    content="Bottom center">&#xe617;</i>
                 </el-popover>-->
               <el-tooltip content="打印邮票"  effect="dark"  v-if="scope.row.whStatus != '待审核'   && ButtonInfo['打印邮票']!='0'" placement="bottom">
                 <i  class="iconfontyyy"   content="Bottom center">&#xe601;</i>
               </el-tooltip>
               <el-tooltip content="修改运费" v-if="scope.row.whStatus == '已发货'  && ButtonInfo['修改运费']!='0'" effect="dark"   placement="bottom" >
                 <i  class="iconfontyyy" @click="dialogFlag.freightisOpen = true;getquerySingle(scope.row.reqId)"  content="Bottom center">&#xe62b;</i>
               </el-tooltip>
               <el-tooltip content="修改跟踪单号"   effect="dark"  placement="bottom" v-if="scope.row.whStatus == '已发货'  && ButtonInfo['修改跟踪单号']!='0'">
     ...
     <div class="smallDia">
       <el-dialog title="筛选" :visible.sync="dialogFlag.sccrenisOpen" size="small" :close-on-click-modal="false"  @open="colseScreen();">
         <div class="data_body">
           ...
           <time-data    :start-time="pagefilters.startDate" :end-time="pagefilters.endDate" @child-value-Stime="getTime" @child-value-Etime="getTime" ></time-data>
         </div>
