  ...
  <div id="outermost_wrap" class="role" style="">
    ...
    <div id="content_wrap" class="content_wrap">
      <el-row class="filterOption" id="filterOption" >
        <el-col :span="8" class="optionValue">
          <el-select v-model="pageFilter.whId" filterable clearable placeholder="请选择仓库" @change="getProcurrementList" size="small" style="width:100%">
      ...
      <el-row class="supplier_btn last_btn">
        <el-button size="small" type="primary"  @click="dialogFlag.dialogTitle='采购单入库';handlerItems()" v-if="pageFilter.state == 1 && ButtonInfo['批量入库']!='0'">批量入库</el-button>
        <el-button size="small" type="primary"  @click="toPrint" v-if="pageFilter.state != '' && ButtonInfo['批量打印']!='0'">批量打印</el-button>
    </el-row>
      <el-table :data="procurrementData" id="item_list" border tooltip-effect="dark" style="width: 100%" @selection-change="selsChange"  ref="clearTable">
    ...
    <el-dialog :title="dialogFlag.dialogTitle" :visible.sync="dialogFlag.infoDetail"  size="small">
      <el-table border :data="warehousingList" >
        ...
        <el-table-column label="SKU/商品名称"  align="center"  width="140px">
          <template scope="scope">
            <span>{{scope.row.skuNo}}</span><br/>
            <span>{{scope.row.proName}}</span><br/>
          </template>
        </el-table-column>
        <el-table-column prop="insAllQty" label="质检结果(良品/次品)" align="center" ></el-table-column>
        <el-table-column prop="wareAllQty" label="已入库(良品/次品)" align="center" ></el-table-column>
        <el-table-column  label="货位号" align="center" width="120px">
          <template scope="scope" >
        ...
        <el-table-column  label="入库" align="center" width="120px">
          <template scope="scope" >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submit" v-if="dialogFlag.dialogTitle == '采购单入库'"  :loading="$store.state.loading">入库</el-button>
      </div>
    </el-dialog>
<!---------------------添加备注弹出框------------------->
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        statusOptions:[{value:'01',label:'待入库'},{value:'02',label:'已入库'}],
        activeNames: ['1'],
        procurrementData: [],
        pageFilter: {
          currentPage: 1,
