<template>
  <!--全部订单-&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;-->
  <div id="outermost_wrap" class="user_manager_wrap">
    <div id="sub_wrap" class="top-nav">
      <CrumbsNav></CrumbsNav>
    </div>
    <div id="content_wrap" class="content_wrap">
      <el-row >
        <el-col :span="2">
          <el-select v-model="inputTitle" filterable  @change="inputValue=''" size="small">
            <el-option v-for="item in options" :key="item.label" :label="item.label" :value="item.label"></el-option>
          </el-select>
        </el-col>
        <el-col :span="4" class="optionValue">
          <el-input v-model="inputValue" :placeholder="inputTitle == '平台订单' ? '请输入平台订单' :  '请输入系统订单'" size="small"></el-input>
        </el-col>
        <el-col :span="2">
          <el-button type="primary" size="small" @click="selectFilter(inputTitle,inputValue)">搜索</el-button>
        </el-col>
        <div style="text-align: right;">
          <el-button type="primary" class="research_btn" size="small"  @click="manualData.isOpen = !manualData.isOpen;">手工同步订单</el-button>
        </div>
      </el-row>
      <!--筛选条件 平台，账号，站点及国家-->
      <el-row id="filer" class="">
        <status-type :channel-id-item="pageFilter.channelId"  :account-no-item="pageFilter.accountNo" :country-no-item="pageFilter.countryNo" :country-item="pageFilter.country" @getParamsData="getParamsData"></status-type>
      </el-row>
      <el-row class="supplier_btn ">
        <el-button type="primary" size="small"  @click="dialogFlag.open = !dialogFlag.open;dialogFlag.filterDialog = true;">筛选</el-button>
        <el-button type="primary" size="small" @click="getSupplyData()" v-if="pageFilter.channelId != ''&& pageFilter.accountNo != ''&& pageFilter.countryNo != ''">审核设置</el-button>
      </el-row>
      <el-row class="supplier_btn last_btn" v-if="pageFilter.channelId == '01' || pageFilter.channelId == '02' ">
        <el-button type="primary" size="small" @click="batchBtnDone('发邮件');"  v-if="pageFilter.channelId == '01'">发邮件</el-button>
         <el-button type="primary" size="small" @click="batchBtnDone('发消息');" v-if="pageFilter.channelId == '02'">发消息</el-button>
        <!--平台等于ebay的时候显示发消息，其他平台都是发邮件-->
        <!--平台等于AliExpress的时候显示发消息，发邮件都不显示-->
       <!-- <el-button type="primary" size="small" @click="batchBtnDone('批量审核');" >批量审核</el-button>-->
      </el-row>
      <div id="item_list">
        <el-table :data="tableData" border tooltip-effect="dark" style="width: 100%"   @selection-change="selsChange">
          <table-column-data></table-column-data>
          <el-table-column label="操作" prop="label" align="center"  width="160px">
            <template scope="scope">
              <el-tooltip content="订单详情"  effect="dark" placement="bottom">
                <i  class="iconfontyyy" content="Bottom center" @click="tool.goToPage('orderDetails?transferId='+scope.row.orderId+'&allowRefund='+scope.row.allowRefund+'&allowRedirect='+scope.row.allowRedirect+'&allowReturn='+scope.row.allowReturn)">&#xe600;</i>
              </el-tooltip>
              <el-tooltip content="添加备注"  effect="dark" placement="bottom">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '添加备注')">&#xe623;</i>
              </el-tooltip>
              <el-tooltip content="发邮件"  effect="dark" placement="bottom">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '发邮件')">&#xe738;</i>
              </el-tooltip>

              <span v-if="scope.row.status != '手工订单'">
                <el-tooltip content="复制创建手工订单"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '复制创建手工订单')">&#xe607;</i>
                </el-tooltip>
              </span>

              <span v-if="scope.row.allowRefund == 1">
                <el-tooltip content="退款"  effect="dark" placement="bottom" >
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '退款')">&#xe690;</i>
                </el-tooltip>
              </span>

              <span v-if="scope.row.allowReturn == 1">
                <el-tooltip content="发起预退件"  effect="dark" placement="bottom" >
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '发起预退件')">&#xe62b;</i>
                </el-tooltip>
              </span>

              <span v-if="scope.row.status == '已付款'">
                <el-tooltip content="提交"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '提交')">&#xe63d;</i>
                </el-tooltip>
                <el-tooltip content="取消绑定"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '取消绑定')">&#xe62a;</i>
                </el-tooltip>
              </span>

              <span v-if="scope.row.status == '异常订单' || scope.row.status == '缺货订单'">
                <el-tooltip content="提单"  effect="dark" placement="bottom" v-if="scope.row.status == '异常订单'">
                 <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '提交')">&#xe63d;</i>
                </el-tooltip>
                <el-tooltip content="作废"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '异常或缺货作废')">&#xe6f7;</i>
                </el-tooltip>
              </span>

              <span v-if="scope.row.status == '手工订单'">
                <el-tooltip content="作废订单"  effect="dark" placement="bottom"  >
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '手工订单作废订单')">&#xe6f7;</i>
                </el-tooltip>
                <el-tooltip content="提交审核"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '手工订单提交审核')">&#xe63d;</i>
                </el-tooltip>
              </span>

              <span v-if="scope.row.status == '待审核(异常)' ||  scope.row.status == '待审核(手工)'">
                <el-tooltip content="取消绑定"  effect="dark" placement="bottom"  v-if="scope.row.status == '待审核(手工)'">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '取消绑定')">&#xe62a;</i>
                </el-tooltip>
                <el-tooltip content="同意"  effect="dark" placement="bottom">
                 <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '同意')">&#xe63d;</i>
                </el-tooltip>
                <el-tooltip content="不同意"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '不同意')">&#xe611;</i>
                </el-tooltip>
              </span>

              <span v-if="scope.row.status == '已审核'">
                <el-tooltip content="回退"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '回退')">&#xe62a;</i>
                 </el-tooltip>
              </span>

           <!--   <span v-if="scope.row.status == '已生成波次' ||  scope.row.status == '已打印'">-->
                <el-tooltip content="拦截订单"  effect="dark" placement="bottom" v-if="scope.row.allowIntercept == 1">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '拦截订单')">&#xe652;</i>
                 </el-tooltip>
            <!--  </span>-->

              <span v-if="scope.row.status == '已发货' ||  scope.row.status == '已签收'">
                <el-tooltip content="重寄申请"  effect="dark" placement="bottom" v-if="scope.row.allowRedirect == 1">
                 <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '重寄申请')">&#xe61c;</i>
                </el-tooltip>
                <el-tooltip content="新增配件申请"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '新增配件申请')">&#xe60d;</i>
                </el-tooltip>
                <el-tooltip content="下载VTA发票"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '下载VTA发票')">&#xe804;</i>
                </el-tooltip>
              </span>

              <!--/*&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;已发货、已签收 已打印，已生成波次，已审核，未付款 手动提交流程按钮-&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;&#45;*/-->
              <span v-if="scope.row.status == '缺货订单' ||　scope.row.status == '已发货' ||  scope.row.status == '已打印'||  scope.row.status == '已签收' ||  scope.row.status == '已生成波次' ||  scope.row.status == '已审核' ||   scope.row.status == '未付款'">
                 <el-tooltip :content="getContent(scope.row.status)"  effect="dark" placement="bottom">
                  <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '检测')">&#xe610;</i>
                </el-tooltip>
              </span>

            </template>
          </el-table-column>
        </el-table>
      </div>
    </div>
    <div id="page" class="page">
      <el-pagination @size-change="changePageSize" @current-change="changeCurrentPage" :current-page="page.currentPage" :page-sizes="[10,20,30,40]" :page-size="page.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="page.totalCount"></el-pagination>
    </div>
    <!------------------手工同步订单 弹出框--------------------------->
    <manual-dialog :id-value="manualData.id" :is-open="manualData.isOpen" @getData="getData"></manual-dialog>
    <!------------------新增小配件 弹出框--------------------------->
    <Accessory :DialogTitle="Accessory.title" :getpoId="Accessory.id" :getdata="Accessory.dialogInfo"  :infoisOpen="Accessory.isOpen"  @Change-list="Accessory.isOpen=false"></Accessory>
    <!------------------重寄申请 弹出框--------------------------->
    <resend-dialog :id-value="resenddData.id" :is-open="resenddData.isOpen" @getData="getData"></resend-dialog>
    <!------------------拦截订单 弹出框--------------------------->
    <intercept-dialog :id-value="interceptData.id" :is-open="interceptData.isOpen" :account-id="pageFilter.accountNo"></intercept-dialog>
    <!------------------退款 弹出框--------------------------->
    <refunds-dialog :id-value="refundsData.id" :is-open="refundsData.isOpen" @getData="getData"></refunds-dialog>
    <!------------------发起预退件 弹出框--------------------------->
    <pre-withdrawal :id-value="preWithdrawal.id" :is-open="preWithdrawal.isOpen" :add-btn="true" dialog-title="发起预退件" @getData="getData"></pre-withdrawal>
    <!------------------复制手工订单 弹出框--------------------------->
    <copy-order-dialog :id-value="copyOrderData.id" :is-open="copyOrderData.isOpen" :dialog-title="copyOrderData.title"  @getData="getData"></copy-order-dialog>
    <!------------------添加备注 弹出框--------------------------->
    <remark-dialog :id-value="remarkData.id" :is-open="remarkData.isOpen" @getData="getData"></remark-dialog>
    <!------------------发消息/邮件 弹出框--------------------------->
    <mail-dialog :id-list="mailData.ids" :is-open="mailData.isOpen" :dialog-title="mailData.title" :channel-id="pageFilter.channelId" @getData="getData"></mail-dialog>
    <!------------------筛选 弹出框--------------------------->
    <el-dialog title="筛选" :visible.sync="dialogFlag.filterDialog" :close-on-click-modal="false" size="small" @open="isGroup=false" @close="isGroup=true;">
      <screen-data :params-data="pageFilter" ref="screenData"  :is-open="dialogFlag.open"></screen-data>
      <div slot="footer" class="dialog-footer">
        <el-button @click="closeFilterDialog">清空筛选</el-button>
        <el-button @click="filterData();" :loading="$store.state.loading">筛选</el-button>
        <el-button @click="dialogFlag.filterDialog = false;">取消</el-button>
      </div>
    </el-dialog>
    <!--  审核设置Dal--------------------------->
    <el-dialog title="审核设置"  :visible.sync="dialogFlag.isOpen" :close-on-click-modal="false" size="small" @close="resetForm('supplyData');supplyData.startTimeStr='';supplyData.endTimeStr='';startTimeStr='';endTimeStr='';">
      <el-form label-width="180px" :model="supplyData"  ref="supplyData">
        <el-form-item label="自动提交审核：" prop="isAuto">
          <el-switch v-model="supplyData.isAuto"></el-switch>
        </el-form-item>
        <el-form-item label="在以下时间开启/关闭：" prop="switchOption">
          <el-select v-model="supplyData.switchOption" placeholder="请选择" style="margin-bottom: 20px;">
            <el-option v-for="item in supplyDataLlist" :key="item.value" :label="item.label" :value="item.value">{{item.label}}</el-option>
          </el-select>
          <el-row class="block">
            <el-col style="width: 200px;">
              <el-time-picker v-model="startTimeStr" editable format="HH:mm:ss" placeholder="任意时间点" @change="getSTime"></el-time-picker>
            </el-col>
            <el-col style="width: 200px;">
              <el-time-picker v-model="endTimeStr" editable format="HH:mm:ss" placeholder="任意时间点" @change="getETime"></el-time-picker>
            </el-col>
          </el-row>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFlag.isOpen = false;">取消</el-button>
        <el-button type="primary" @click="saveSetting('supplyData')" :loading="$store.state.loading">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
<script>
  import { mapState,mapMutations,mapGetters,mapActions } from 'vuex'
  export default{
    components: {},
    data(){
      return {
        supplyData:{   // 审核设置 子组件传参
          isAuto:true,
          switchOption:'',
          startTimeStr:'',
          endTimeStr:'',
        },
        startTimeStr:'',
        endTimeStr:'',
        //supplyDataLlist:[{value:'1',label:'一直开启'},{value:'2',label:'一直关闭'},{value:'3',label:'一直以下开启'},{value:'4',label:'一直以下关闭'},],
        supplyDataLlist:[{value:'0',label:'一直开启/关闭'},{value:'1',label:'一直以下时间开启/关闭'}],
        mailData:{   // 发邮件/消息 子组件传参
          ids:[],
          isOpen:false,
          title:'',
        },
        remarkData:{  // 添加血液 子组件传参
           id:'',
          isOpen:false,
        },
        copyOrderData:{  // 复制/新建手工订单 子组件传参
           id:'',
          title:'',
          isOpen:false,
        },
        refundsData:{  // 退款 子组件传参
           id:'',
          isOpen:false,
        },
        manualData:{  // 手工同步订单 子组件传参
           id:'',
          isOpen:false,
        },
        preWithdrawal:{  // 发起预退件 子组件传参
          id:'',
          isOpen:false,
        },
        interceptData:{  // 拦截订单 子组件传参
          id:'',
          isOpen:false,
        },
        resenddData:{  // 重寄申请 子组件传参
          id:'',
          isOpen:false,
        },
        Accessory:{  // 新增配件申请 子组件传参
          id:'',
          title:'新增配件申请',
          isOpen:false,
          dialogInfo:false,
        },
        inputTitle:'系统订单',
        inputValue:'',
        options: [{value: 1, label: '系统订单'}, {value: 2, label: '平台订单'}],
        isOpen:false,   // 触发 子组件 消费弹窗是否打开的标识
        activeNames: ['1'],
        sels: [],   // table选中数据
        tableData:[],   // 表格数据
        pageFilter:{
          currentPage:1,
          pageSize:10,
          status:'全部',   //  节点
          orderNo:'',  //  系统  订单号
          shopOrderId:'',  // 平台  订单号
          channelId:'',
          accountNo:'',
          countryNo:'',
          toCountryNo:'',
          customerInfo:'',
          logisticsMethod:'',
          whId:'',
          skuNo:'',
          salesUser:'',
        },
        status:'',
        page:{          // 分页参数
          currentPage:1,
          pageSize:10,
          totalPage:10,
          totalCount:10,
          isChange:true,
        },
        dialogFlag:{
          isOpen:false,     // 审核设置
          open:false,     //
          filterDialog:false,  //  筛选弹出框
          putMoneyFlag:false,     // 充值
          infoDetail:false,    //  添加 编辑  查看框
          remarkDialog:false,    //  添加 编辑  查看框
          inputDisabled:false,   //checkbox是否处理可点击状态
        },
        financeFormLabelWidth: '140px',
        isGroup:true,
      }
    },
    computed:{

    },
    mounted () {
      this.getData();
    },
    methods: {
      getContent:function(val){
        return val == '缺货订单' ? '检测缺货' : val == '已发货' ? '计算利润' : val == '已打印' ? '检测发货' : val == '已签收' ? '结算提成' : val == '已生成波次' ? '检测打印' : val == '已审核' ? '检测是否可全并' : val == '未付款' ? '检测付款' : '' ;
      },
      getSTime:function(val){
          this.supplyData.startTimeStr = val;
      },
      getETime:function(val){
        this.supplyData.endTimeStr = val;
      },
      getSupplyData:function(){  // 加载审核设置
        this.dialogFlag.isOpen = true;
        var params = {
          channel:this.pageFilter.channelId,
          accountId:this.pageFilter.accountNo,
          countryNo:this.pageFilter.countryNo,
        }
        this.httpGet('orderAutoSubmitCfg/select',params,'Sms').then(res =>{
          if(res.data){
            this.supplyData = res.data;
            this.startTimeStr = this.supplyData.startTimeStr;
            this.endTimeStr = this.supplyData.endTimeStr;
          }
        })
      },
      saveSetting:function(formName){  // 保存审核设置
        this.dialogFlag.isOpen = true;
        this.supplyData.channel = this.pageFilter.channelId,
        this.supplyData.accountId = this.pageFilter.accountNo,
        this.supplyData.countryNo = this.pageFilter.countryNo,
        this.httpPost('/orderAutoSubmitCfg/save',this.supplyData,'Sms').then(res =>{
          this.$message(res.data.message);
          if(res.data.result == true){
            this.dialogFlag.isOpen = false;
          }
        })
      },
      selectFilter:function(title,val){
        this.pageFilter.orderNo = title == '系统订单' ? val :'';
        this.pageFilter.shopOrderId = title == '平台订单' ? val :'';
        this.getData();
      },
      getData:function(){
        var vm = this;
        this.page.isChange = false;
        this.httpPost('orderInfo/selectList',this.pageFilter,'Sms').then(res =>{
          this.page = res.data;
          this.tableData=res.data.listData;
          setTimeout(function(){
            vm.page.isChange = true;
          },100)
          this.dialogFlag.filterDialog = false;
        })
      },
      selsChange:function(sels){
         this.sels = sels;
      },
      batchBtnDone:function(type){    // 批量操作
        let vm=this;
        var commonMail = function(title){
          if(vm.sels.length < 1){
            vm.tips();
          }else{
            vm.mailData.title = title;
            vm.mailData.ids = vm.sels.map(item =>item.orderId);
            vm.mailData.isOpen = !vm.mailData.isOpen;
          }
        }
        switch(type){
          case '发邮件':
            commonMail('发邮件');
            break;
          case '发消息':
            commonMail('发消息');
           break;
          case '批量审核':
            var params = [];
            this.sels.forEach(function(item){
                params.push({nodeId:item.nodeId,orderId:item.orderId,unbound:0})
            })
            vm.sels.length<1 ? vm.tips():vm.doItem('您确定要提交该订单的审核？','orderInfo/completeMultiTask',params,vm.getData);
            break;
        }
      },
      rowBtnDone:function(row,type) {   // 行内操作
        let vm=this;
        switch(type){
          case '添加备注':
            this.remarkData.id = row.orderId;
            this.remarkData.isOpen = !this.remarkData.isOpen;
            break;
          case '发邮件':
            this.mailData.title = '发邮件';
            this.mailData.ids = [];
            this.mailData.ids.push(row.orderId);
            this.mailData.isOpen = !this.mailData.isOpen;
            break;
          case '复制创建手工订单':
            this.copyOrderData.id = row.orderId;
            this.copyOrderData.title = '复制创建手工订单';
            this.copyOrderData.isOpen = !this.copyOrderData.isOpen;
            break;
          case '退款':
            this.refundsData.id = row.orderId;
            this.refundsData.isOpen = !this.refundsData.isOpen;
            break;
          case '发起预退件':
            this.preWithdrawal.id = row.orderId;
            this.preWithdrawal.isOpen = !this.preWithdrawal.isOpen;
            break;

          /*------------已付款订单 独享按钮 （异常订单共用 提交 按钮 ）（待审核(手工) 共享  取消绑定 按钮）-----------------------------------------------------------*/
          case '提交':
            var params = {nodeId:row.nodeId,orderId:row.orderId,unbound:0}
            vm.doItem('您确定要提交该订单的审核？','orderInfo/completeTask',params,vm.getData);
            break;
          case '取消绑定':
            var params = {nodeId:row.nodeId,orderId:row.orderId,unbound:1,agree:0};
             vm.doItem('您确定该订单认领付款的需要解绑？解绑后该订单进入未付款。','orderInfo/unboundPay',params,vm.getData);
            break;

            /*------------手工订单 独享按钮-------------------------------------------------------------------------------*/
          case '手工订单提交审核':
            var params = {nodeId:row.nodeId,orderId:row.orderId,cancel:0}
            vm.doItem('您确定提交该订单进行审核？','orderInfo/completeTask',params,vm.getData);
            break;
          case '手工订单作废订单':
            var params = {
              nodeId:row.nodeId,
              orderId:row.orderId,
              cancel:1,
            }
            vm.doItem('作废订单确定取消您确定要作废该订单？作废后将移入已作废订单，请谨慎操作！','orderInfo/completeTask',params,vm.getData);

            /*------------异常订单 独享按钮-----------------------------------------------------------------------------------------*/
          case '异常或缺货作废':
            var params = {nodeId:row.nodeId,orderId:row.orderId,cancel:1};
            vm.doItem('您确定作废该订单？','orderInfo/cancelEbayOrder',params,vm.getData);
            break;
          case '提单':
            var params = {nodeId:row.nodeId,orderId:row.orderId,cancel:0};
            vm.doItem('您确定提单该订单？','orderInfo/completeTask',params,vm.getData);
            break;

            /*------------待审核 下的异常 及手工状态 下 独享按钮-----------------------------------------------------------------------------------------*/
          case '同意':
            var params = {nodeId:row.nodeId,orderId:row.orderId,agree:1};
            vm.doItem('您确定同意该订单？','orderInfo/completeTask',params,vm.getData);
            break;
          case '不同意':
            var params = {nodeId:row.nodeId,orderId:row.orderId,agree:0};
            vm.doItem('您确定不同意该订单？','orderInfo/completeTask',params,vm.getData);
            break;

            /*------------已审核 独享按钮-----------------------------------------------------------------------------------------*/
          case '回退':
            var params = {nodeId:row.nodeId,orderId:row.orderId,toAbnormal:1};
            vm.doItem('您确定要回退该订单到异常订单？','orderInfo/completeTask',params,vm.getData);
            break;

            /*------------已生成波次、已打印 独享按钮-----------------------------------------------------------------------------------------*/
          case '拦截订单':
            this.interceptData.id = row.orderId;
            this.interceptData.isOpen = !this.interceptData.isOpen;
            break;

            /*------------已发货、已签收 独享按钮-----------------------------------------------------------------------------------------*/
          case '重寄申请':
            this.resenddData.id = row.orderId;
            this.resenddData.isOpen = !this.resenddData.isOpen;
            break;
          case '新增配件申请':
            this.preWithdrawal.id = row.orderId;
            this.Accessory.isOpen = true;
            break;
          case '下载VTA发票':
            var url = this.RmsUrl+'orderVatInvoice/exportVatInvoice?orderId='+row.orderId+'&Authorization='+sessionStorage.getItem('userSession');
            window.open(url);
            break;

            /*------------缺货订单，已发货、已签收 已打印，已生成波次，已审核，未付款 手动提交流程按钮-----------------------------------------------------------------------------------------*/
          case '检测':
            var params = {nodeId:row.nodeId,orderId:row.orderId};
            vm.doItem('您确定提交该订单？','orderInfo/completeTask',params,vm.getData);
            break;
        }
      },
      changePageSize:function(val){
        if(this.page.isChange == true){
          this.pageFilter.pageSize=val;
          this.pageFilter.currentPage = 1;
          this.getData();
        }
      },
      changeCurrentPage:function(val){
        if(this.page.isChange == true) {
          this.pageFilter.currentPage = val;
          this.getData();
        }
      },
      resetForm(formName) {
        this.$refs[formName].resetFields();
      },
      closeFilterDialog:function(){
        this.$refs.screenData.clearData();
      },
      filterData(){
        var data = this.$refs.screenData.returnData();
        console.log(data);
        console.log(111111111111)
        this.pageFilter.channelId = data.channelId;
        this.pageFilter.countryNo = data.countryNo;
        this.pageFilter.accountNo = data.accountNo;
        this.pageFilter.orderNo = data.orderNo;   //  系统订单号？
        this.pageFilter.customerInfo = data.customerInfo;
        this.pageFilter.logisticsMethod = data.logisticsMethod;
        this.pageFilter.whId = data.whId;
        this.pageFilter.skuNo = data.skuNo;
        this.pageFilter.salesUser = data.salesUser;
        this.getData();
      },
      getParamsData:function(val){
        console.log(val)
        console.log(22222222222);
        if(this.isGroup == true){
          var data = JSON.parse(JSON.stringify(val.params));
          this.pageFilter.channelId = data.channelId;
          this.pageFilter.countryNo = data.countryNo;
          this.pageFilter.accountNo = data.accountNo;
          this.pageFilter.toCountryNo = data.toCountryNo;
          this.getData();
        }
      },
    }
  }
</script>
<style>
.mailInfo{
  border-top:1px solid #dfe6ec;
  margin-top:10px;
  padding-top:10px;
}
  #border_bt{
    border-top:1px solid #d1dbe5 ;
  }
  .paddingStyle{
    padding:5px 0;
  }
</style>
