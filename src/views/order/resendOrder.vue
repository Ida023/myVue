    ...
    <div id="content_wrap" class="content_wrap">
      ...
      <el-row>
        <el-tabs v-model="pageFilter.status" @tab-click="getData">
          <el-tab-pane label="待审核" name="待审核(重寄)"></el-tab-pane>
          <el-tab-pane label="等待退件" name="等待退件"></el-tab-pane>
          <el-tab-pane label="已生成新订单" name="已生成新订单"></el-tab-pane>
          <el-tab-pane label="已重寄" name="已重寄"></el-tab-pane>
        </el-tabs>
      </el-row>
      ...
      <el-row class="supplier_btn last_btn">
        ...
        <el-button type="primary" size="small" @click="batchBtnDone('批量同意');" v-if="pageFilter.status=='待审核(重寄)'">批量同意</el-button>
        <el-button type="primary" size="small" @click="batchBtnDone('批量不同意');" v-if="pageFilter.status=='待审核(重寄)'">批量不同意</el-button>
      </el-row>
      <div id="item_list">
        <el-table :data="tableData" border tooltip-effect="dark" style="width: 100%"   @selection-change="selsChange">
          <table-column-data :is-show="pageFilter.status == '已生成新订单' ? true : pageFilter.status == '已重寄' ? true : false "></table-column-data>
          <el-table-column label="操作"  prop="label" align="center"  width="160px">
            <template scope="scope">
              ...
              <el-tooltip content="下载VTA发票"  effect="dark" placement="bottom" v-if="pageFilter.status=='已生成新订单'|| pageFilter.status=='等待退件'">
              ...
              <el-tooltip content="同意"  effect="dark" placement="bottom"  v-if="pageFilter.status=='待审核(重寄)'">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '同意')">&#xe63d;</i>
              </el-tooltip>
              <el-tooltip content="不同意"  effect="dark" placement="bottom" v-if="pageFilter.status=='待审核(重寄)'">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '不同意')">&#xe611;</i>
              </el-tooltip>
             <!-- <el-tooltip content="取消重寄"  effect="dark" placement="bottom" v-if="pageFilter.status!='已重寄'">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '取消重寄')">&#xe666;</i>
              </el-tooltip>-->
            </template>
          </el-table-column>
        </el-table>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      getData:function(){
        this.httpPost('/orderRedirect/selectFlowList',this.pageFilter,'Sms').then(res =>{
         this.page = res.data;
      ...
      batchBtnDone:function(type){    // 批量操作
        ...
        switch(type){
          ...
          case '发消息':
            ...
            break;
          case '批量同意':
            vm.sels.length<1 ? vm.tips():vm.doItem('您确定同意该订单？','orderRedirect/completeMultiTask',params,vm.getData);
            break;
          case '批量不同意':
            vm.sels.length<1 ? vm.tips():vm.doItem('您确定不同意该订单？','orderRedirect/completeMultiTask',params,vm.getData);
            break;
        }
      },
      rowBtnDone:function(row,type) {   // 行内操作
        let vm=this;
        var agree = type == '同意' ? 1 : 0;
        ...
        switch(type){
          ...
          case '下载VTA发票':
            ...
            break;
          case '同意':
            vm.doItem('您确定同意该订单？','orderRedirect/completeTask',params,vm.getData);
            break;
            case '不同意':
            vm.doItem('您确定不同意该订单？','orderRedirect/completeTask',params,vm.getData);
            break;
          case '取消重寄':
            vm.doItem('您确定要取消该订单的重寄？取消之后该订单将进入已发货/已签收列表。。','ceshiurl',row.orderId,vm.getData);
            break;
        }
      },
