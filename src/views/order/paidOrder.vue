<template>
  ...
  <div id="outermost_wrap" class="user_manager_wrap">
    ...
    <!--  审核设置Dal--------------------------->
    <el-dialog title="审核设置"  :visible.sync="dialogFlag.isOpen" :close-on-click-modal="false" size="small" @close="resetForm('supplyData');supplyData.startTimeStr='';supplyData.endTimeStr='';">
  ...
  export default{
    ...
    methods: {
      ...
      saveSetting:function(formName){  // 保存审核设置
        this.dialogFlag.isOpen = true;
        this.supplyData.channelId = this.pageFilter.channelId,
      ...
      rowBtnDone:function(row,type) {   // 行内操作
        ...
        switch(type){
          ...
          case '提交':
            ...
            break;
          case '取消绑定':
            var params = {nodeId:row.nodeId,orderId:row.orderId,unbound:1};
