  ...
  <div id="outermost_wrap" class="user_manager_wrap">
    ...
    <div id="content_wrap" class="content_wrap">
      ...
      <el-row class="supplier_btn">
        <el-button class="el-icon-plus" type="primary" size="small"  @click="copyOrderData.isOpen = !copyOrderData.isOpen;">手工订单</el-button>
        <el-button type="primary" size="small"  @click="importData.isOpen = !importData.isOpen;">导入订单</el-button>
          ...
          <el-table-column label="操作"  prop="label" align="center"  width="160px">
            <template scope="scope">
              ...
              <el-tooltip content="作废订单"  effect="dark" placement="bottom"  >
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '作废订单')">&#xe6f7;</i>
              </el-tooltip>
              <el-tooltip content="提交审核"  effect="dark" placement="bottom">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '提交审核')">&#xe63d;</i>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
    ...
    <!------------------导入手工订单 弹出框--------------------------->
    <import-dialog  :is-open="importData.isOpen" :import-url="importData.importUrl" :modal-url="importData.modalUrl" @getData="getData"></import-dialog>
    <!------------------筛选 弹出框--------------------------->
...
<script>
  ...
  export default{
    components:{bankTransferDialog,importDialog},
    data(){
      return {
        ...
        copyOrderData:{  // /新建手工订单 子组件传参
           id:'',
          title:'新建手工订单',
          isOpen:false,
        },
        importData:{  // /新建手工订单 子组件传参
          importUrl:'orderInfo/importOrder',
          modalUrl:'orderInfo/templateExport',
          isOpen:false,
        },
        inputTitle:'系统订单',
        inputValue:'',
