  ...
  <div id="outermost_wrap" class="user_manager_wrap">
    ...
    <div id="content_wrap" class="content_wrap">
      ...
      <el-row class="supplier_btn last_btn">
        ...
        <el-button type="primary" size="small" @click="batchBtnDone('批量回退');" >批量回退</el-button>
      </el-row>
      <div id="item_list">
        <el-table :data="tableData" border tooltip-effect="dark" style="width: 100%"   @selection-change="selsChange">
          ...
          <el-table-column label="操作"  prop="label" align="center"  width="160px">
            <template scope="scope">
              ...
              <el-tooltip content="检测是否可合并"  effect="dark" placement="bottom">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '检测是否可合并')">&#xe610;</i>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
          ...
          case '批量回退':
            ...
            if(this.sels.length < 1){
                ...
                this.sels.forEach(function(item){
                    params.push({nodeId:row.nodeId,orderId:row.orderId,toAbnormal:1});
                })
              vm.sels.length<1 ? vm.tips():vm.doItem('您确定要回退该订单到异常订单？','orderInfo/completeMultiTask',params,vm.getData);
            }
            break;
        }
      },
