    ...
    <div id="content_wrap" class="content_wrap">
      ...
      <el-row>
        <el-tabs v-model="pageFilter.status" @tab-click="getData">
          <el-tab-pane name="待审核(拦截)" label="拦截"></el-tab-pane>
          <el-tab-pane name="待审核(异常)" label="异常"></el-tab-pane>
          <el-tab-pane name="待审核(退款)" label="退款"></el-tab-pane>
          <el-tab-pane name="待审核(重寄)" label="重寄"></el-tab-pane>
          <el-tab-pane name="待审核(手工)" label="手工"></el-tab-pane>
        </el-tabs>
      </el-row>
      ...
      <el-row class="supplier_btn last_btn">
        <el-button type="primary" size="small" @click="batchBtnDone('发邮件');"  v-if="pageFilter.channelId != '02' && pageFilter.channelId != ''">发邮件</el-button>
        <el-button type="primary" size="small" @click="batchBtnDone('发消息');" v-if="pageFilter.channelId == '02' && pageFilter.channelId != ''">发消息</el-button>
        <!-- <el-button type="primary" size="small" @click="batchBtnDone('发消息');">发消息</el-button>-->
        <!--平台等于ebay， AliExpress的时候显示发消息，其他平台都是发邮件-->
        <el-button type="primary" size="small" @click="batchBtnDone('批量同意');">批量同意</el-button>
        <el-button type="primary" size="small" @click="batchBtnDone('批量不同意');">批量不同意</el-button>
      </el-row>
      <div id="item_list">
        <el-table :data="tableData" border tooltip-effect="dark" style="width: 100%"   @selection-change="selsChange">
          ...
          <el-table-column label="操作"  prop="label" align="center"  width="160px">
            <template scope="scope">
              ...
              <el-tooltip content="退款"  effect="dark" placement="bottom" v-if="pageFilter.status!='待审核(拦截)' && pageFilter.status!='待审核(退款)' && scope.row.allowRefund == 1">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '退款')">&#xe690;</i>
              </el-tooltip>
              <el-tooltip content="重寄申请"  effect="dark" placement="bottom" v-if="pageFilter.status=='退款订单' && scope.row.allowRedirect == 1">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '重寄申请')">&#xe61c;</i>
              </el-tooltip>
              <el-tooltip content="取消绑定"  effect="dark" placement="bottom"  v-if="pageFilter.status=='待审核(手工)'">
                <i  class="iconfontyyy" content="Bottom center" @click="rowBtnDone(scope.row, '取消绑定')">&#xe62a;</i>
              </el-tooltip>
              <el-tooltip content="发起预退件"  effect="dark" placement="bottom" v-if="pageFilter.status!='待审核(拦截)' && pageFilter.status!='待审核(手工)' && scope.row.allowReturn == 1" >
  ...
  export default{
    ...
    data(){
      return {
        ...
        isGroup:true,
        getDataUrl:'', // 进入页面所请求的url
        singleAuditeUrl:'', //  不同状态下单个审核所调用的url
        batchAuditeUrl:'', // 不同状态下批量审核所调用的url
      }
    },
    computed:{
    },
    ...
    methods: {
      ...
      getUrl:function(){   // 获取不同状态下的http请求url
        var status = this.pageFilter.status;
        if(status == '待审核(拦截)'){
          this.getDataUrl = 'orderIntercept/selectFlowList';
          this.singleAuditeUrl = 'orderIntercept/completeTask';
          this.batchAuditeUrl = 'orderIntercept/completeMultiTask';
        }else  if(status == '待审核(异常)' || status == '待审核(手工)'){
          this.getDataUrl = 'orderInfo/selectList';
          this.singleAuditeUrl = 'orderInfo/completeTask';
          this.batchAuditeUrl = 'orderInfo/completeMultiTask';
        }else if(status == '待审核(退款)'){
          this.getDataUrl = 'orderRefund/selectFlowList';
          this.singleAuditeUrl = 'orderRefund/completeTask';
          this.batchAuditeUrl = 'orderRefund/completeMultiTask';
        }else if(status == '待审核(重寄)'){
          this.getDataUrl = 'orderRedirect/selectFlowList';
          this.singleAuditeUrl = 'orderRedirect/completeTask';
          this.batchAuditeUrl = 'orderRedirect/completeMultiTask';
        }
      },
      getData:function(){
        this.getUrl();
        this.httpPost(this.getDataUrl,this.pageFilter,'Sms').then(res =>{
          this.page = res.data;
      ...
      batchBtnDone:function(type){    // 批量操作
        ...
        switch(type){
          ...
          case '发消息':
            ...
            break;
          case '批量同意':
            vm.sels.length<1 ? vm.tips():vm.doItem('您确定同意该订单？',vm.batchAuditeUrl,params,vm.getData);
            break;
          case '批量不同意':
            vm.sels.length<1 ? vm.tips():vm.doItem('您确定不同意该订单？',vm.batchAuditeUrl,params,vm.getData);
            break;
        }
      },
      rowBtnDone:function(row,type) {   // 行内操作
        ...
        switch(type){
          ...
          case '发起预退件':
            ...
            break;
          case '同意':
            var params = {nodeId:row.nodeId,orderId:row.orderId,agree:1,requestId:row.requestId};
            vm.doItem('您确定同意该订单？',vm.singleAuditeUrl,params,vm.getData);
            break;
          case '不同意':
            var params = {nodeId:row.nodeId,orderId:row.orderId,agree:0,requestId:row.requestId};
            vm.doItem('您确定不同意该订单？',vm.singleAuditeUrl,params,vm.getData);
            break;
          case '取消绑定':
