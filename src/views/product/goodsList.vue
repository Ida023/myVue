  ...
  <div id="outermost_wrap">
    ...
    <div id="content_wrap">
      ...
      <el-row class="marginBottom" style="border-bottom:1px solid #bfcbd9;padding-bottom: 10px; ">
        <el-button size="small"  type="primary"  v-if="ButtonInfo['添加商品']!='0'" @click="dialogFlag.inputDisabled = false;dialogFlag.prlblemask =false;getaddCreateIssues(dialogFlag.addDialog  = false);">添加商品</el-button>
        <el-button size="small"  type="primary" v-if="ButtonInfo['添加组合商品']!='0'" @click="dialogFlag.inputDisabled = false;dialogFlag.prlblemask =false;assistlist.dialoglist=[];getaddCreateIssues(dialogFlag.addDialog = true);">添加组合商品</el-button>
...
<!--
        <el-button size="small"  type="primary" @click="batchSales('','1','classify');classifyData.batchAdjustment = true;classifyData.title='批量调整分类';dialogFlag.inputDisabled=false">批量调整分类</el-button>
-->
        <el-button size="small"  type="primary" v-if="ButtonInfo['批量分配销售']!='0'" @click="houseAssigned('','1');skuAssigned.inputDisabled = false;">批量分配销售</el-button>
      </el-row>
      <el-table  id="item_list" class="publicCell"   border v-model="listData" tooltip-effect="dark" :data="listData" label-width=""  @selection-change="handleSelectionChange"  style="word-wrap:break-word;word-break:break-all;">
        ...
        <el-table-column  id="space" label="操作"  align="center"  width="125px" style="word-wrap:break-word;word-break:break-all;">
          <template scope="scope">
            <el-tooltip content="查看"  effect="dark" placement="bottom" v-if="ButtonInfo['查看']!='0'">
              <i  class="iconfontyyy" @click="getCreateIssues(scope.row,dialogFlag.inputDisabled = true);dialogFlag.prlblemask=true; assistlist.dialoglist=[];dialogFlag.addDialog  = false;" content="Bottom center">&#xe600;</i>
            </el-tooltip>
            <el-tooltip content="编辑"  effect="dark" placement="bottom"   v-if="ButtonInfo['编辑']!='0'">
              <i  class="iconfontyyy" @click="getCreateIssues(scope.row,dialogFlag.inputDisabled = false);dialogFlag.prlblemask=true;assistlist.dialoglist=[]; dialogFlag.addDialog  = false;" content="Bottom center">&#xe6e5;</i>
            </el-tooltip>
        ...
        <el-table-column label="SKU"   width="125px"  prop="skuNo" align="center"  sortable  ></el-table-column>
        <el-table-column label="old SKU"   width="125px"  prop="oldSkuNo" align="center"  sortable  ></el-table-column>
        <el-table-column label="图片" prop="imageUrl" width="125px"  align="center" sortable >
          <template scope="scope">
        ...
        <el-table-column label="EAN"   width="100px" prop="eanCode"  sortable align="center"></el-table-column>
        <el-table-column label="分类"  sortable align="center" width="280">
          <template scope="scope" >
        ...
        <el-table-column label="产品开发"  width="80px"  align="center"  prop="developUser" sortable >
        </el-table-column>
        <el-table-column label="采购跟单"  width="80px"  align="center"  prop="purchaseUser" sortable ></el-table-column>
        <el-table-column label="状态"  width="75px"  align="center"  prop="salesStatus"  sortable ></el-table-column>
        <el-table-column label="仓库与销售" width="80px" align="center"   sortable >
          <template scope="scope">
      ...
      <el-dialog :title="dialogFlag.dialogTitle" :visible.sync="dialogFlag.isOpen" :close-on-click-modal="false" @close="ResetFrom">
        <el-tabs v-model="dialogFlag.activeTab"  @tab-click="tabclick">
          ...
          <el-tab-pane label="辅助信息" :disabled="!dialogFlag.docmentNull"  name="1">
            <assist-dialog  ref="assistDialog" :group-product="dialogFlag.addDialog" :has-data="hasdataa"  :product-id="pagefilters.productId"  :assidialoglist="assistlist.dialoglist"   :assistparentinfo-detail="dialogFlag.activeTab"
                            :input-disabled="dialogFlag.inputDisabled"  v-on:parentmsggl="assiparentmsgg"  v-on:assdialog="getassdialog" :docment-null="dialogFlag.docmentNull"  @parent-full="getchildFull"  :fullListt="fullList" ></assist-dialog>
          </el-tab-pane>
          <el-tab-pane label="采购信息"  :disabled="!dialogFlag.docmentNull" name="2">
             <purchase-dialog ref="purchaseDialog"    :purchaseparentinfo-detail="dialogFlag.activeTab" :product-id="pagefilters.productId"
                              :input-disabled="dialogFlag.inputDisabled" v-on:parentmsggll="purparentmsgg"  :docment-null="dialogFlag.docmentNull"></purchase-dialog>
          </el-tab-pane>
          ...
          <el-tab-pane label="物流方式"   name="5"  :disabled="!dialogFlag.docmentNull">
            <logistics-dialog ref="logisticsDialog" :logisticsparentinfo-detail="dialogFlag.activeTab"  @assignment2="Logisticsupdate" @assignment="getLogisticsVlaue" :input-disabled="dialogFlag.inputDisabled" :product-id="pagefilters.productId"></logistics-dialog>
          </el-tab-pane>
    ...
    <div id="page" class="block page">
      ...
      <import-dialog  :is-open="importData2.isOpen" :import-url="importData2.importUrl" :modal-url="importData2.modalUrl" @getData="requestem"></import-dialog>

    </div>
  </div>
</template>
<script>
  import { msgDialog,assistDialog,purchaseDialog,describeDialog,imagesDialog,logisticsDialog,screenDialog,getAllpersonnel} from 'views/product/import_modular';
        ...
        data(){
            return {
              importData2:{
                ...
                modalUrl:'productLogisticsLeadIn/exportModel',
              },
              importData:{
                isOpen:false,
                importUrl:'productLeading/importProductInfo',
      ...
      this.getauthorityList();
       ...
       //this.exportUrl=this.RmsUrl+'product/exportProductDate?Authorization='+sessionStorage.getItem('userSession')+'&salesStatus='+this.pagefilters.salesStatus;
        ...
        methods: {
          ...
          Logisticsupdate(){
          	this.importData2.isOpen=!this.importData2.isOpen;
          },
        	//导出
          exportList(url){
         ...
         /* handleCheckAllChange(item) {
          ...
          //新增
          getaddCreateIssues(val){
          	//alert(!this.dialogFlag.docmentNull)
            this.dialogFlag.docmentNull =false;
            this.dialogFlag.isOpen=true;
            this.dialogFlag.dialogTitle = val ? '添加组合产品' : '添加商品';
            this.dialogFlag.activeTab = '0';
            setTimeout(()=>{
