<template>
    <div class="selectorCom">
      <el-select v-model="selectVal" filterable clearable :placeholder="placeholder" :multiple="isMultiple" :disabled="disabled" :no-data-text="noData" :type="type" :size="ssize">
        <el-option v-for="item in options" :label="item.corporationName" :key="item.corporationId" :value="item.corporationId"></el-option>
      </el-select>
    </div>
</template>
<script>
  export default{
    ...
    methods: {
      getDataList(){
          /*获取公司列表*/
        this.httpPost('financialCorporation/select',{isUse:1},'Sms').then(res =>{
